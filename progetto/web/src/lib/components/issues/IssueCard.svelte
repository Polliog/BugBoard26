...
>
	<div class="flex items-start gap-3 sm:gap-4">
		<div class="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 hidden sm:block">
			...
			{:else}
				<div class="w-full h-full rounded-lg bg-gray-100 flex items-center justify-center">
					<svg class="w-5 h-5 sm:w-6 sm:h-6 text-gray-400" aria-hidden="true" fill="none" stroke="currentColor" viewBox="0 0 24 24">
		...
		<div class="flex-1 min-w-0">
			<h3 class="text-base sm:text-lg font-semibold text-gray-900 line-clamp-1 mb-1.5">{issue.title}</h3>
			...
			<p class="text-sm text-gray-600 mb-2 line-clamp-2 hidden sm:block">{issue.description}</p>
