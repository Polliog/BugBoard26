...
<div class="bg-white rounded-xl shadow-sm p-4 sm:p-6">
	<h2 class="text-xl sm:text-2xl font-bold text-gray-900 mb-4 sm:mb-6">Commenti ({comments.length})</h2>
				...
				<div class="border border-gray-200 rounded-lg p-3 sm:p-4">
					<div class="flex items-start gap-2 sm:gap-3">
						<div class="w-8 h-8 sm:w-10 sm:h-10 bg-blue-100 rounded-full flex items-center justify-center flex-shrink-0">
							<span class="text-blue-700 font-semibold text-xs sm:text-sm">{comment.author.name.charAt(0)}</span>
						</div>
						<div class="flex-1 min-w-0">
							<div class="flex items-start justify-between mb-2">
								<div>
									<p class="font-medium text-gray-900 text-sm sm:text-base">{comment.author.name}</p>
	...
	{#if canComment}
		<div class="pt-4 sm:pt-6 border-t border-gray-200">
			<h3 class="text-base sm:text-lg font-semibold text-gray-900 mb-3">Aggiungi Commento</h3>
			<form onsubmit={(e) => { e.preventDefault(); handleAdd(); }}>
				...
				<textarea id="new-comment" bind:value={newContent} rows="3" placeholder="Scrivi un commento..."
					class="w-full px-3 py-2 sm:px-4 sm:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none mb-3"></textarea>
				...
				<div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2">
					<label class="inline-flex items-center justify-center gap-2 px-3 py-2 text-sm text-gray-600 border border-gray-300 rounded-lg hover:bg-gray-50 cursor-pointer">
						<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
						Allega immagine
